...
def rfe():
    ...
    for i in range(n):
        ...
        inf[i].extend(imp_fea)
        n-=1
        imp_fea.clear
    df = pd.DataFrame(columns=['number','features'])
    ...
    df.to_csv('../reduction/rfe_01.csv',index=False)
    return df
...
def cv_rf(combs):
    ...
    df.to_csv('../cv/cv_rfe_01.csv',index=False)
...
#        gb = GradientBoostingClassifier(n_estimators=200, random_state = 0)
    ...
    df.to_csv('../test/rf_rfe_01.csv',index=False)
    return inf
