...
#    from sklearn.ensemble import ExtraTreesClassifier
    ...
    for feature in zip(feat_labels,clf.feature_importances_):
        x.append(feature)
        print(feature)
    x = sorted(x,key=lambda l:l[1], reverse=True)
    x.reverse()
    
    imp_fea=[]
#    imp_fea.append(feat_labels)
    for i in range(X_train.shape[1]):
        imp_fea.append(tuple(feat_labels))
        feat_labels.remove(x[i][0])
    
    n = [[num+1] for num in range(n)]
    n.reverse()
    
    for i in range(len(imp_fea)):
        imp_fea[i] = list(imp_fea[i])

    df = pd.DataFrame({'number of features':n, 'features':imp_fea, 'selection time':selection_time})
    
    df.to_csv('../reduction/imp_01.csv',index=False)
...
def cv_rf(combs):
    ...
    df.to_csv('../cv/cv_imp_01.csv',index=False)
...
def rf(combs):
    ...
    df.to_csv('../test/rf_imp_01.csv',index=False)
    return inf
